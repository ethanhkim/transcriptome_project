...
if (methodToUse == "hyper") {
  ...
  He_genelist <- He_DS1_Human_averaged %>%
    dplyr::select(gene_symbol, Layer_3_marker) %>%
    filter(!is.na(Layer_3_marker)) %>%
    dplyr::select(gene_symbol) %>%
    pull()
  names(He_genelist) <- He_genelist 
...
if (methodToUse == "AUC") {
  ...
  top_go_term <- result %>%
    filter(AUC > 0.5) %>% dplyr::select(ID) %>% slice_head() %>% mutate(ID = gsub(",.*", "", ID)) %>% as.character()
