...
databaseToUse <- "DisGeNet" #to indicate that you want to use Phenocarta, or "DisGeNet" to indicate you want to use DisGeNet
layerToExamine <- "Layer_4"
...
if (methodToUse == "AUC") {
  ...
  if (datasetToUse == "Maynard") {
    Maynard_genelist <- Maynard_dataset_average %>%
      dplyr::select(gene_symbol, Layer_5_marker) %>%
      filter(!is.na(Layer_5_marker)) %>%
      dplyr::select(gene_symbol) %>%
      pull()
    names(Maynard_genelist) <- Maynard_genelist 
